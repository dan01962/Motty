select
    ...
    pfm.FTHG      'FTHG',
    pfm.FTAG      'FTAG',
    case when (pfm.FTHG + pfm.FTAG) = 0  then 'U' else 'O' end 'UO0P5',
    case when (pfm.FTHG + pfm.FTAG) <= 1 then 'U' else 'O' end 'UO1P5',
    case when (pfm.FTHG + pfm.FTAG) <= 2 then 'U' else 'O' end 'UO2P5',
    case when (pfm.FTHG + pfm.FTAG) <= 3 then 'U' else 'O' end 'UO3P5',
    concat('H',
			case when pfm.FTHG < 4 then pfm.FTHG else 'X' end,
            'A',
            case when pfm.FTAG < 4 then pfm.FTAG else 'X' end) 'Score',
	case when pfm.ftr = 'D' then 'Y' else 'N' end 'DRAW',
	case when pfm.ftr = 'H' then 'Y' else 'N' end 'HOME',
	case when pfm.ftr = 'A' then 'Y' else 'N' end 'AWAY'
